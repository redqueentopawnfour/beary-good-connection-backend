...
router.get("/weatherParams", (req, res) => { 
    ...
    const options = new URL('https://api.openweathermap.org/data/2.5/weather?lat={47.6}&lon={-122.33}&APPID=256b3fac9d8ec8ce35e6be9487360e9c');
...
router.get("/weatherForecast", (req, res) => { 
  ...
  const options = new URL('https://api.weatherbit.io/v2.0/forecast/daily?lat=47.606&lon=-122.33&units=I&days=10&key=f991b0a6c72941ecb4103f79eee8a9f2');
  //const options = new URL(`https://api.weatherbit.io/v2.0/current?lat=${lat}&lon=${lon}&key=f991b0a6c72941ecb4103f79eee8a9f2`);
  http.get(options, (resp) => {
      let responseString = '';
      resp.on('data', function(data) {
